...
parser.add_argument('--outfile_prefix', type=str, help='Prefix for output video paths', 
...
def process_audio_file(audio_path, full_frames, fps, face_det_results, model):
    output_path = args.outfile_prefix + os.path.basename(audio_path).rsplit('.', 1)[0] + '.mp4'
    
    # Extract wav from audio file if needed
    ...
    # Important fix: Make sure we handle frame count properly
    ...
    # Create extended face_det_results if needed by cycling through existing results
    ...
    # Create frame loop if needed to match audio length
...
def main():
    ...
    if args.box[0] == -1:
        if not args.static:
            ...
            # Duplicate the result for all frames if static
    ...
    os.makedirs(os.path.dirname(args.outfile_prefix), exist_ok=True)
